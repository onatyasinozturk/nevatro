        ...
        {provinces.map((p) => (
          <Reveal key={p.slug} className="mb-10">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {p.districts.map((d) => <Link key={d.slug} href={`/hizmet-bolgeleri/${p.slug}/${d.slug}`} className="px-3.5 py-2 rounded-lg bg-surface border border-line text-sm hover:border-primary">{d.name}</Link>)}
            </div>
          </Reveal>
        ))}
      </section>
